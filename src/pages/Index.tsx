...
export default function Index() {
  ...
  useEffect(() => {
    async function fetchServerIp() {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/serverinfo/ip`,
        );
  ...
  const hasFilters = useMemo(
    () => genre || status || language || searchQuery,
    [genre, status, language, searchQuery],
  );
  ...
  return (
    <div className="flex h-screen bg-background text-foreground">
      ...
      <div className="flex-1 flex flex-col">
              ...
              <div className="space-y-12">
                {Object.entries(groupedBooks).map(([genre, booksOnShelf]) => (
                  <div key={genre}>
                    ...
                    <div
                      className="h-4 bg-yellow-700/30 dark:bg-yellow-900/50 mt-2 rounded-md shadow-inner"
                      style={{
                        backgroundImage:
        ...
        <div className="flex flex-col md:flex-row p-3 border-t">
          <div
            className={`text-sm font-bold border px-1 py-2 border-white ${
              serverIp === "Gagal Mengedentifikasi Alamat IP"
                ? "text-red-500"
          ...
          <div
            className={`text-sm font-bold ml-3 border px-1 py-2 border-white ${
              serverIp === "Gagal Mengedentifikasi Alamat IP"
                ? "text-red-500"
                : "text-black dark:text-white"
            }`}
